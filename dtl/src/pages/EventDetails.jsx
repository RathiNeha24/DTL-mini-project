...
const GlowOrb = ({ x, y, color = "#1a3aff", size = 300 }) => (
  <div style={{
...
const SeatBar = ({ registered, seats }) => {
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: COLORS.muted, marginBottom: 5 }}>
      ...
      <div style={{ height: 4, borderRadius: 4, background: "#1e2d6e", overflow: "hidden" }}>
        <div style={{
          ...
          background: full ? "#e24b4a" : `linear-gradient(90deg, ${COLORS.persian}, ${COLORS.accent})`,
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center", zIndex: 1 }}>
          <div style={{ fontSize: 42, marginBottom: 4 }}>
            {event.category === "Hackathon" ? "⟨/⟩" : event.category === "Conference" ? "◈" :
             event.category === "Design" ? "◉" : event.category === "Competition" ? "⛬" :
             event.category === "Entrepreneurship" ? "◆" : "∿"}
          </div>
          <div style={{ fontSize: 11, color: COLORS.muted, letterSpacing: "0.12em", textTransform: "uppercase" }}>
            {event.organizer}
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: "16px 18px 18px", flex: 1, display: "flex", flexDirection: "column", gap: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          {event.registered >= event.seats && (
            <span style={{ fontSize: 11, color: "#e24b4a", fontWeight: 600 }}>FULL</span>
          )}
        </div>
 
        <div>
          <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: COLORS.white, lineHeight: 1.3, fontFamily: "'Syne', sans-serif" }}>
            {event.title}
          </h3>
        </div>
...
const EventsListing = ({ onSelect }) => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: COLORS.navy, fontFamily: "'Inter', sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <GlowOrb x="-100px" y="-100px" size={500} />
      <GlowOrb x="60%" y="200px" color="#00d4ff" size={400} />
      <GlowOrb x="30%" y="600px" color="#6b21a8" size={350} />
 
      {/* Nav */}
      <nav style={{
        position: "sticky", top: 0, zIndex: 100,
        background: "#0a0f2ccc", backdropFilter: "blur(20px)",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 20, color: COLORS.white }}>
            Event<span style={{ color: COLORS.persianLight }}>Ease</span>
          </span>
        </div>
        <div style={{ display: "flex", gap: 28, fontSize: 14, color: COLORS.muted }}>
          <span style={{ color: COLORS.persianLight, fontWeight: 600, cursor: "pointer" }}>Events</span>
      ...
      <div style={{ position: "relative", zIndex: 1, padding: "70px 40px 50px", maxWidth: 1100, margin: "0 auto" }}>
        <div style={{
          ...
          borderRadius: 20, border: `1px solid ${COLORS.persianLight}44`,
          background: COLORS.persianGlow, fontSize: 13, color: COLORS.persianLight,
          letterSpacing: "0.08em",
        }}>SPRING SEMESTER 2025</div>
        <h1 style={{
          fontFamily: "'Syne', sans-serif", fontWeight: 800,
          fontSize: "clamp(36px, 5vw, 58px)", margin: "0 0 16px",
          color: COLORS.white, lineHeight: 1.1, letterSpacing: "-0.02em",
        }}>
          Upcoming <span style={{
        ...
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center" }}>
          <div style={{ position: "relative", flex: "1 1 260px" }}>
            <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: COLORS.muted, fontSize: 16 }}>⌕</span>
            <input
              value={search}
              ...
              style={{
                ...
                color: COLORS.white, fontSize: 14, outline: "none", boxSizing: "border-box",
              }}
            />
          </div>
      ...
      <div style={{ position: "relative", zIndex: 1, padding: "0 40px 80px", maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 24 }}>
          {filtered.map(e => <EventCard key={e.id} event={e} onSelect={onSelect} />)}
        </div>
        {filtered.length === 0 && (
...
const EventDetails = ({ event, onBack }) => {
  ...
  const handleRegister = () => {
    if (full || registered) return;
    setAnimating(true);
    setTimeout(() => { setRegistered(true); setAnimating(false); }, 600);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: COLORS.navy, fontFamily: "'Inter', sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <GlowOrb x="-100px" y="100px" color={event.color} size={600} />
      <GlowOrb x="50%" y="400px" color="#00d4ff" size={400} />
      ...
      <nav style={{
        ...
        <button onClick={onBack} style={{
          ...
          borderRadius: 10, background: COLORS.cardBg, border: `1px solid ${COLORS.cardBorder}`,
      ...
      <div style={{
        ...
        <div style={{ position: "absolute", right: 80, top: "50%", transform: "translateY(-50%)", opacity: 0.08, fontSize: 200, fontFamily: "'Syne', sans-serif", fontWeight: 800, color: "#fff", userSelect: "none", whiteSpace: "nowrap" }}>
          {event.category.toUpperCase()}
        </div>
        <div style={{ position: "relative", zIndex: 1, padding: "0 60px", maxWidth: 1100, margin: "0 auto", width: "100%" }}>
          <CategoryBadge cat={event.category} />
          <h1 style={{
            fontFamily: "'Syne', sans-serif", fontWeight: 800, margin: "14px 0 12px",
            fontSize: "clamp(28px, 4vw, 48px)", color: COLORS.white, lineHeight: 1.1,
          }}>{event.title}</h1>
      ...
      {/* Main content */}
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1100, margin: "0 auto", padding: "50px 60px 80px", display: "grid", gridTemplateColumns: "1fr 340px", gap: 40, alignItems: "start" }}>
 
        {/* Left */}
        <div>
          <section style={{ marginBottom: 40 }}>
            <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 22, color: COLORS.white, margin: "0 0 16px" }}>About This Event</h2>
            <p style={{ color: COLORS.muted, fontSize: 16, lineHeight: 1.8, margin: 0 }}>{event.description}</p>
          </section>
 
          <section style={{ marginBottom: 40 }}>
            <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 22, color: COLORS.white, margin: "0 0 20px" }}>Event Highlights</h2>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              {event.highlights.map((h, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ width: 8, height: 8, borderRadius: "50%", background: `linear-gradient(135deg, ${event.color}, ${COLORS.accent})`, flexShrink: 0 }} />
                  <span style={{ color: COLORS.white, fontSize: 14, fontWeight: 500 }}>{h}</span>
                </div>
              ))}
            </div>
          </section>
          ...
          <section>
            <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 22, color: COLORS.white, margin: "0 0 16px" }}>Tags</h2>
        ...
        <div style={{ position: "sticky", top: 80 }}>
          <div style={{
            background: COLORS.cardBg, border: `1px solid ${COLORS.cardBorder}`,
            borderRadius: 20, padding: 28, boxShadow: `0 20px 60px #00000060`,
          }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 24 }}>
              {[
                ...
                <div key={r.label} style={{ display: "flex", justifyContent: "space-between", fontSize: 14 }}>
                  ...
                  <span style={{ color: COLORS.white, fontWeight: 500, textAlign: "right", maxWidth: 180 }}>{r.val}</span>
                </div>
              ))}
            </div>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 12, border: "none", cursor: full || registered ? "default" : "pointer",
          ...
          {/* Share strip */}
          <div style={{
            marginTop: 14, padding: "16px 20px", borderRadius: 14,
            background: COLORS.cardBg, border: `1px solid ${COLORS.cardBorder}`,
            display: "flex", alignItems: "center", justifyContent: "space-between",
            fontSize: 14,
          }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              {["𝕏", "in", "🔗"].map(s => (
                <button key={s} style={{
                  ...
                  background: COLORS.navyMid, border: `1px solid ${COLORS.cardBorder}`,
                  color: COLORS.white, cursor: "pointer", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center",
                }}>{s}</button>
              ))}
            </div>
          </div>
